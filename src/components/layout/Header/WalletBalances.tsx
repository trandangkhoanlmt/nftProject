...
export const WalletBalances = ({
  ...
  const runApp = async () => {
    const url =
      ...
      "/nft?chain=" +
      "0xaa36a7" +
      "&format=decimal&media_items=true";
    const response = await axios.get(url, {
      headers: {
        Accept: "application/json",
        "X-API-Key":
          "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJub25jZSI6ImFlMzdiMTBmLTkzNDQtNDJkYi04YzEzLWRmOTg2MTUwODY1YSIsIm9yZ0lkIjoiMzQyMzY0IiwidXNlcklkIjoiMzUxOTU1IiwidHlwZUlkIjoiMGI3OGVlNzAtYjYxZC00MzA3LTlkMGYtMjBkNWYyYWJhZmFlIiwidHlwZSI6IlBST0pFQ1QiLCJpYXQiOjE2ODg5ODE4NTUsImV4cCI6NDg0NDc0MTg1NX0.DkgaujHVjFNtMVAdpoWXRvo9rRfyFpHpQ9yiOPNHCiw",
      },
    });
  ...
  async function allCawNfts() {
    ...
    for (let i = 0; i < ((allNfts ?? "").length ?? 0); i++) {
      const nft = allNfts?.[i];
      if (
        nft?.token_address?.toLowerCase() === USERNAME_ADDRESS?.toLowerCase()
      ) {
        console.log(nft);
        const contract = new web3.eth.Contract(
          USERNAME_ABI as unknown as any,
          USERNAME_ADDRESS
        );
        const data = await contract.methods
          .getProfileImageURI(allNfts?.[i]?.token_id)
          .call();
        const response = await axios.get(data);
        cawNfts.push({
          ...response?.data,
          tokenId: allNfts?.[i]?.token_id,
          tx: allNfts?.[i]?.transaction_hash,
        });
      }
    }
    setCawNfts(cawNfts);
    ...
    >
      <Box sx={style}>
        ...
        <Box sx={{ textAlign: "-webkit-center" }}>
          {cawNfts ? (
            cawNfts.map((nft) => {
              const image = nft?.image?.replaceAll(
                "ipfs://",
                "https://ipfs.io/ipfs/"
              );
              ...
              return (
                <Box>
                  <Chip
                    sx={CHIP}
                    color={
                      decodeProfile() && decodeProfile()?.name === nft.name
                        ? "success"
                        : "default"
                    }
                  ...
                  >
                    ...
                    <MenuItem
                      onClick={() =>
                        window.open(
                          OPENSEA + "/" + USERNAME_ADDRESS + "/" + nft.tokenId,
                          "_blank"
                        )
                      }
                    >
                    ...
                    <MenuItem
                      onClick={() =>
                        window.open(
                          "https://sepolia.etherscan.io/nft" +
                            "/" +
                            USERNAME_ADDRESS +
                            "/" +
                            nft.tokenId,
                    ...
                    <MenuItem onClick={() => genareteHashed(nft)}>
                      Change Account
                    </MenuItem>
                  </Menu>
                </Box>
