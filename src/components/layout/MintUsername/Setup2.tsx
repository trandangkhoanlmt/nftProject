...
export const Setup2 = () => {
  ...
  return (
    <Box>
      ...
      <Typography sx={TITLE}>MY BALANCES</Typography>
      {faucetData && (
        <Box sx={{ mt: 3 }}>
          {faucetData.status === 400 && (
            <Alert severity="error">{faucetData.data}</Alert>
          )}
          {faucetData.status === 200 && (
            <Alert severity="success">1000000 tCAW sent.</Alert>
          )}
        </Box>
      )}
      ...
      <Grid sx={{ my: "31px" }} container>
        ...
        <Tokens
          col={12}
          tokenAddress={"0xf3b9569F82B18aEf890De263B84189bd33EBe452"}
          imgSize={"25px"}
          img={"/assets/img/logo.png"}
          setup={2}
        />
      ...
      <Box sx={DESC}>
        <AddNetwork
          ...
          explorer={[
            chain?.blockExplorers?.default ?? "https://sepolia.etherscan.io/",
          ]}
        />
      ...
      <Box sx={{ mb: "58px" }} display={"flex"} justifyContent={"center"}>
        {faucetLink?.map((data, index: number) => (
          <Box
            key={index}
            target={"_blank"}
            sx={A}
        ...
        <Box
          sx={{ ...A, border: "none", cursor: "pointer" }}
          disabled={manuelDisaled}
          onClick={() => tCawFaucet()}
          component={"button"}
        >
          tCAW Token
